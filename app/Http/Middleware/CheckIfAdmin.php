...
class CheckIfAdmin
{
     ...
     * when trying to access an admin route. By default it's '/home' but Starmoozie
    ...
    private function respondToUnauthorizedRequest($request)
    {
        if ($request->ajax() || $request->wantsJson()) {
            return response(trans('starmoozie::base.unauthorized'), 401);
        } else {
            return redirect()->guest(starmoozie_url('login'));
        }
    }
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (! $this->checkIfUserIsAdmin(starmoozie_user())) {
            return $this->respondToUnauthorizedRequest($request);
        }
